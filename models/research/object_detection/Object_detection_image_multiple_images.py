...
IMAGE_PATHS = [cv2.imread(file) for file in glob.glob(os.path.join(CWD_PATH,"faces\\*.jpg"))]
#print(glob.glob(os.path.join(CWD_PATH,"faces\\*.jpg")))
...
print(count)
file = glob.glob(os.path.join(CWD_PATH,"faces\\*.jpg"))
for i in range(0,len(IMAGE_PATHS)):

    FILENAME_TIMESTAMP= "\n" + str(file[i]) +"#" + str(os.path.getctime(file[i])) + "#"
    with open("C:\\tensorflow1\\models\\research\\object_detection\\ID_Detection_Output.txt", "a") as oup1:
    ...
    print(count)
    fish_det="Person Count: "+str(count[i])
...
print("Total Coconut Count:", sum(count))
text_file = open("C:\\tensorflow1\\models\\research\\object_detection\\Coconut_Count_Output.txt", "w")
